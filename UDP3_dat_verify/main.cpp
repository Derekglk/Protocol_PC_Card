...
void *t_trait (void * buff){
  ...
  long clk_tck = CLOCKS_PER_SEC;
    ...
    serverAddr.sin_addr.s_addr =33597632;
    serverAddr.sin_port = htons(7892);

    //Initialisation de la taille de l'adresse du serveur
...
void *t_rec (void * buff){
    ...
    //Mémorisation de l'adresse du client
    memset(serverAddr.sin_zero, '\0', sizeof serverAddr.sin_zero);  

    //Liaison entre la socket et l'adress du serveur
    ...
    while((*r).k<nb){
      ...
      //Verouillage du mutex en attente de la réception complète du paquet*/
        ...
        nBytes = recvfrom(udpSocket,(*r).buffer[(*r).k],960*sizeof(int),0,(struct sockaddr *)&serverStorage, &addr_size);

        //Mesure du temps de la réception du premier paquet
		if((*r).k==0){
		    t1 = clock();
		}
		/****************************************************************/
