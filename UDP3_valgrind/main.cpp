...
void *t_trait (void * buff){
  ...
  long sortie[nb][TEC_FRAME_LG_WB*3];
  long tab[TEC_FRAME_LG_WB*3]; //Tableau pour mettre en forme les échantillons avant de les envoyer
  ...
  long clk_tck = CLOCKS_PER_SEC;
    ...
    serverAddr.sin_addr.s_addr =16777343; // Ici l'adresse du client. En c++ on n'utilise pas la fonction inet_addr("IP") on écrit directement la valeur de l'adresse IP
	...
	for (n1 = 0; n1 < nb; n1++)
	{
	  /*Verouillage du mutex et attente de la réception du paquet entier*/
		...
		sendto(udpSocket,tab,960*sizeof(long),0,(struct sockaddr *)&serverAddr,addr_size);	     
...
void *t_rec (void * buff){
    ...
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_addr.s_addr =16777343; // Ici c'est l'adesse du serveur. En c++ on n'utilise pas la fonction inet_addr("IP") on écrit directement la valeur de l'adresse IP.
