...
void *t_env(void * buff){
	...
	buffer_rec = (int **) malloc(nb*sizeof(int* ));
	...
	for(i=0;i<nb;i++){
		buffer_rec[i] = (int *) malloc(960*sizeof(int));
...
void *t_rec (void * buff){
    ...
    int ** b; // buffer où l'on va stocker les échantillons débruités
